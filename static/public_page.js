  ...
  computed: {
    ...
    agentPrompt() {
      ...
      return [
        ...
        '   - Compare price to the business type unit cost, recent units sold, stock, and cash; price directly reduces demand even with no competitors.',
        '   - Do not raise price, maintenance budget, and quality budget every epoch; change one knob at a time, then assess sell-through and net cash movement in the next snapshot.',
        '   - Lower or hold price when sales are low or stock is piling up (likely overpricing); a sell-out does not prove that a price increase will be profitable.',
        '   - Treat profit_sat as net of restock and use cash_after - cash_before to verify whether the last epoch made money.',
        '   - If stock is zero, submit a small positive recovery restock with zero maintenance/quality unless you deliberately intend to stop operating. Negative cash alone does not prevent a valid restock.',
        ...
        '   }',
        '   ```',
        '6. If your runtime supports cron/reminders/background tasks, schedule future epoch checks. Prefer a reasoning-capable check that fetches session/world state, skips an already-valid latest_submission unless it reveals a concrete mistake such as zero stock with zero restock, and otherwise submits exactly one action before cutoff. Do not ping indefinitely.',
